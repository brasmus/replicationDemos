...
Scafetta2012 <- function() {
  ...
  plot(yymm,t2m,type="l",col="red",xlim=c(1850,2050),ylim=c(-0.8,1.2),
       main="After Scafetta (2012): Fig. 5b",
  ...
  text(2000,1.20,"S2012: anomaly by",pos=4,cex=0.8)
...
Scafetta.tab1 <- function() {
  ...
  attr(A,'description')= "60-year amplitude"
  attr(B,'description')= "20-year amplitude"
  attr(C,'description')= "trend (deg C/decade)"

  results <- list(A=A,a=Scafetta2011.tab1$a[-1]*0.10,
                  B=B,b=Scafetta2011.tab1$b[-1]*0.040,
                  C=C*10,c=Scafetta2011.tab1$c[-1]*0.10)
  attr(results,'description') <- "Scafetta.tab1() [replicationDemos; R]"
    
  dev.new(); par(bty="n",col.axis="white")
  plot(c(0,7),c(-0.5,0.25),type="n",ylim=c(-0.05,0.25),
          main="Scafetta (2012): Table 1",xlab="coeffiecient",
